...
/**
 * @brief 获取卡号到ucArray_ID；
 *        
 *        
 * @note  无返回值，在程序中更改ucArray_ID。
 *        
 * @param ucArray_ID用于存放卡号的数组
 * @retval void
 */

u8  Get_ucArray_ID(uint8_t * ucArray_ID ,u8 interface_code )
{		 unsigned char ucStatusReturn;
		
		if(interface_code==Main_Interface)
		{if ( ( ucStatusReturn = PcdRequest ( PICC_REQALL, ucArray_ID ) ) != MI_OK )
			ucStatusReturn = PcdRequest ( PICC_REQALL, ucArray_ID ); //进行寻卡，要先寻卡再用防冲突get卡号
	...
	return MI_ERR;}
		if(interface_code==IC_add_Interface)   
					{if ( ( ucStatusReturn = PcdRequest ( PICC_REQALL, ucArray_ID ) ) != MI_OK )
					ucStatusReturn = PcdRequest ( PICC_REQALL, ucArray_ID ); //进行寻卡，要先寻卡再用防冲突get卡号
				if ( ucStatusReturn == MI_OK)
				{
			...
			if(interface_code==IC_Verify_Interface)   
					{if ( ( ucStatusReturn = PcdRequest ( PICC_REQALL, ucArray_ID ) ) != MI_OK )
					ucStatusReturn = PcdRequest ( PICC_REQALL, ucArray_ID ); //进行寻卡，要先寻卡再用防冲突get卡号
				if ( ucStatusReturn == MI_OK)
				{
